# Codex Blog - Blog Creation & Optimization Skill

## Project Overview

This repository contains **Codex Blog**, a Tier 4 Codex skill for blog content
...
Codex-blog/
  ...
  .Codex-plugin/plugin.json         # Plugin manifest (v1.9.0)
  .Codex-plugin/marketplace.json    # Marketplace catalog for distribution
...
- SKILL.md frontmatter: only valid fields (name, description, user-invokable, argument-hint, compatibility, license, metadata, disable-model-invocation). Do NOT use `allowed-tools`; it is not a Codex spec field
...
- Run `Codex plugin validate .` before pushing plugin changes
...
Submit at: Codex.ai/settings/plugins/submit or platform.Codex.com/plugins/submit

### Self-Hosted Marketplace
```
/plugin marketplace add AgriciDaniel/Codex-blog
/plugin install Codex-blog@AgriciDaniel-Codex-blog
```

### Standalone Install (no marketplace)
```bash
curl -sL https://raw.githubusercontent.com/AgriciDaniel/Codex-blog/main/install.sh | bash
```
...
This generates a blog post on https://Codex-blog.md/blog/, handles cover image generation, SEO metadata, FAQ schema, internal linking, sitemap/llms.txt updates, Vercel deployment, and Google indexing.
